...
// scene.fog = null;
    ...
    window.addEventListener('keydown', function(e){
      ...
      // don't move camera / change scene when user is typing in inputs or selects
      if(isEditing){
        return;
      }
...
// === Project Save/Load (multi-project, local; backend hook-ready) ===
(function(){
  ...
  var modalSave   = $('projectModalSave');
  var modalCancel = $('projectModalCancel');
  var backdrop    = modal && modal.querySelector('.project-modal-backdrop');

  if (!select || !saveBtn || !loadBtn || !modal || !nameInput || !modalSave || !modalCancel) {
    return;
  }

  var STORAGE_KEY = 'c3d_projects_v1';
  var LAST_KEY    = 'c3d_last_project_v1';
  var projects    = {};
  ...
  function hasProjects() {
    for (var k in projects) {
      if (projects.hasOwnProperty(k)) return true;
    }
    return false;
  }
  ...
  function refreshSelect() {
    ...
    if (!names.length) {
      // no projects yet: hide dropdown and disable load
      select.innerHTML = '';
    ...
    // there are projects: show dropdown and load button
    ...
    names.sort(function(a, b) {
      var ta = (projects[a] && projects[a].savedAt) || 0;
      var tb = (projects[b] && projects[b].savedAt) || 0;
      return tb - ta;
    });
    ...
    for (var i = 0; i < names.length; i++) {
      var n = names[i];
      var sel = (n === last) ? ' selected' : '';
      html += '<option value="' + n.replace(/"/g, '&quot;') + '"' + sel + '>' + n + '</option>';
    }
    select.innerHTML = html;

    // if last name not set or missing, fall back to first
  ...
  function ensureSceneFns() {
    return (typeof snapshotScene === 'function' && typeof restoreScene === 'function');
  }

  // --- modal helpers ---
  function openModal() {
    var base = getLastName();
    if (!base && !select.disabled && select.value) {
      base = select.value;
    }
  ...
  // keyboard handling inside project-name modal
  nameInput.addEventListener('keydown', function(ev){
    ...
    // Остальные клавиши (включая WASD) работают как обычный текст:
    // сцена не двигается, потому что глобальный keydown
    // игнорирует события из input/textarea/select.
  });
...
// --- save/load logic ---
  ...
  function performSave() {
    if (!ensureSceneFns()) {
      if (typeof msg === 'function') msg('Project save not ready', false);
      return;
    }

    var name = normalizeName(nameInput.value);
    if (!name) {
      if (typeof msg === 'function') msg('Введите имя проекта', false);
      nameInput.focus();
      return;
    }
    ...
    if (typeof msg === 'function') msg('Проект "' + name + '" сохранён', true);
    closeModal();

    // TODO: сюда добавить fetch(...) с { name, snapshot } в личный кабинет
  }

  function performLoad() {
    if (!ensureSceneFns()) {
      if (typeof msg === 'function') msg('Project load not ready', false);
      return;
    }

    if (!hasProjects()) {
      if (typeof msg === 'function') msg('Нет сохранённых проектов', false);
      return;
    }

    var name = select.value;
    if (!name || !projects[name] || !projects[name].snapshot) {
      if (typeof msg === 'function') msg('Выберите проект', false);
      return;
    }

    var entry = projects[name];
    restoreScene(entry.snapshot);

    // аккуратно обновляем undo/redo без изменения их логики
    try {
    ...
    setLastName(name);
    if (typeof msg === 'function') msg('Проект "' + name + '" загружен', true);

    // TODO: при переходе на backend можно грузить snapshot по имени с сервера
  }
  ...
  modalSave.addEventListener('click', function(ev){
    ev.preventDefault();
    performSave();
  });

  modalCancel.addEventListener('click', function(ev){
    ev.preventDefault();
    closeModal();
  });
  ...
  try {
    window.projectStore = {
      getAll: function(){
        try { return JSON.parse(JSON.stringify(projects)); }
        catch (e) { return projects; }
      },
      load: performLoad,
      saveFromUI: function(name){
